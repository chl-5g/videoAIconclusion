...
def main() -> int:
    ...
    logger.info("[1/3] 提取音频（FFmpeg）...")
    extract_wav_16k_mono(video, wav_path)
    logger.info("已写入：%s", wav_path)

    logger.info("[2/3] 语音转文本（Faster-Whisper）...")
    ...
    logger.info("检测/使用语言：%s%s", detected, (f"（解码语言：{lang}）" if lang else "（解码语言：自动）"))

    if (lang and lang.lower() == "zh") or detected == "zh":
        segments = segments_simplified_chinese(segments)
        logger.info("已转为简体中文（UTF-8 文本）。")

    plain = segments_to_plain_text(segments)

    plain_single_line = re.sub(r"\s+", " ", plain).strip()
    (out_dir / f"{job_name}.txt").write_text(plain_single_line, encoding="utf-8")
    logger.info("已写入：%s.txt（已去时间戳/换行，句间空格分隔）", job_name)

    if not args.summarize:
        logger.info("[3/3] 跳过总结（--summarize 未开启；默认只产出 mp4/wav/txt）。")
        return 0
    if not os.environ.get("OPENAI_API_KEY"):
        logger.warning("[3/3] 已启用 --summarize 但未设置 OPENAI_API_KEY，跳过。")
        return 0

    logger.info("[3/3] 生成总结（OpenAI 兼容 API）...")
    ...
    logger.info("已写入：%s", conclusion_path)
    return 0
